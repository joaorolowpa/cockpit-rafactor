...
import { OnOffClassificationForm } from "@/components/forms/assets/OnOffClassificationForm";
...
interface Asset {
  ...
  onoff_onshore: number;
  onoff_offshore: number;
  onoff_other: number;
}

export const onoffColumns: ColumnDef<Asset>[] = [
  {
    header: "Actions",
    id: "actions",
    cell: ({ row }) => {
      ...
      return (
        <>
          ...
          <ReusableSheet
            ...
            title="Add OnShore / OffShore Classification"
            description="Add a new classification to this asset"
            variant="default"
          >
            <OnOffClassificationForm data={asset} setIsOpen={setIsSheetOpen} />
          </ReusableSheet>
        </>
      );
    },
  ...
  {
    id: "onoff_onshore",
    accessorKey: "onoff_onshore",
    header: "Onshore",
    cell: ({ row }) => {
      const value = row.getValue("onoff_onshore") as string | null;
      return formatPercentageValue(value);
    },
  },
  {
    id: "onoff_offshore",
    accessorKey: "onoff_offshore",
    header: "Offshore",
    cell: ({ row }) => {
      const value = row.getValue("onoff_offshore") as string | null;
      return formatPercentageValue(value);
    },
  },
  {
    id: "onoff_other",
    accessorKey: "onoff_other",
    header: "Other",
    cell: ({ row }) => {
      const value = row.getValue("onoff_other") as string | null;
      return formatPercentageValue(value);
    },
  },
];
